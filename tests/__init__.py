...
def clear_stream(stream: TextIO):
    """Clear's the content's of a given stream
...
def delete_file(file_path: str):
    """Eliminate's a file if it exists.
    Args:
...
def reset_store(store: FileStorage, file_path='file.json'):
    """Reset's the item's in the allocated store.
    Args:
...
def read_text_file(file_name):
    """Read the content's of a given file.
...
def write_text_file(file_name, text):
    """Write's a text to a given file.
